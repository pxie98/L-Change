...
class dataGenerator:
    ...
    def generate_data(self):
        ...
        points = np.array([[x1, x2, x3] for x1 in x1_vals for x2 in x2_vals for x3 in x3_vals])

        # 生成训练数据
        X = torch.tensor(points, dtype=torch.float)
        y = torch.tensor([self.function(x[0], x[1], x[2]) for x in points], dtype=torch.float).view(-1, 1)
        return X, y