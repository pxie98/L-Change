...
#
# Hyperparameter configuration class for transfer learning experiments
class Argument:
    def __init__(self):
...
for coff in coffs:
    ...
    for layer_num in args.layer_num_list:
        for hidden_size in args.hidden_size_list:
            ...
            #model = adjust_model(model, x, y)  # Scale adjustment (commented out)
            ...
            # Phase 1: Initial training on base function
            losses, model = train.train(model, optimizer, criterion, x, y, test_data, test_labels, 
                                      layer_num, hidden_size, args.path[i], round)
            print('Layer num is {}, hidden_size is {}, loss is {}'.format(layer_num, hidden_size, losses[-1]))

            '''
            # Transfer learning from pre-trained model (commented out)
            path_5x = 'model_x4'
            epoch = 20000
            ...
            model.load_state_dict(torch.load(file_name, map_location=torch.device('cuda')))
            '''

            # Phase 2: Transfer learning on scaled function
            losses, model = train.train(model, optimizer, criterion, x2, y2, test_data, test_labels, 
                                      layer_num, hidden_size, args.path[i], round)

            # Save training results to file
            save_file_name = 'func3_pretrain2/results/test_loss_{}.txt'.format(i)
